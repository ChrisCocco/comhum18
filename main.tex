...
Based on the chosen colour sets, the entropy measure is sensitive to the errors of pixels colour attributions (e.g. similar distances, no Brown, etc.). Then, we compare it with a common approach of Computer Vision which consists to measure the entropy on the histogram greyscale levels of the drawing with the bin $g = 1, \dots, G$. (After the linear conversion of the pixel, we estimated and summed each probability that a grey value appears using the absolute variety as the number of unique grey value. )

On Figure~\ref{fig:entropies}
