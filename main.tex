...
Based on the constrained colour sets chosen, the entropy measure is sensitive to the errors of pixels colour attribution (e.g. similar distances, no Brown, etc.). Then, we compare it with a common approach of Computer Vision which consists to measure the entropy on the greyscale level of the drawing. After the linear conversion of the pixel, we estimated and summed each probability that a grey value appears using the absolute variety as the number of unique grey value. 
