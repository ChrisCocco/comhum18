...
% Je n'ai volontairement pas ajouté de note pour le brun, car on l'explique déjà bien à la fin de la section 3.2 et que ça n'a pas dérangé les reviewers
...
Besides the adoption of another colour space\footnote{The fact to use the RGB colour space (and not another one such as the Munsell colour system, allow to avoid an additional step of transformation.)} and of another colour set, the particularity of our approach is that it uses a two-stage assignment process. Each pixel is first associated with a ``micro-colour'' (represented by one of the 117 small rectangular areas in Figure~\ref{fig:colour_set}), which in turn belongs to a ``macro-colour'' (red, orange, yellow, green, cyan, blue, purple, pink, white, and achromatic). Decomposing colour identification in this way offers three advantages. Firstly, micro-colours are more fine-grained and permit thus to grasp more shades of each colour.
Secondly, it is possible to create a new set of macro-colours without modifying the set of micro-colours as explained at the end of this section.
Thirdly, the fact to associate each pixel with a micro-colour (see equation~\eqref{eq_micro_col}), which are then grouped, allows to not need to define directly the boundaries of each macro-colour (this advantage is also available for the method of ~\citet{kimbaelee2007}).
...
Therefore, the identification of colours depends on two levels of choice which were made by the project team, firstly for the micro-colours and secondly for the macro-colours.
...
%{\color{green}[AX: je ne comprends pas ce dernier point: si l'algo voit 10 couleurs quand les humains en voient 1 et vice-versa, on aura $\overline{d}=0$, non?]} Oui, en effet, ce serait un biais qui se compense, methode pas parfaire, mais c'est pour ça qu'on montre tous les points j'imagine.
...
We would like to thank the reviewers for their constructive feedbacks. We would also like to thank Zhargalma Dandarova Robert, Gr\'{e}gory Dessart, Olga Serbaeva and Zahra Astaneh for their help in the definition of the macro-colours, as well as Christine Mohr and Domicele Jonauskaite for the stimulating discussions about colours in general and about colour definition in particular.
