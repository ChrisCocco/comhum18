...
categories, there are several works from linguistic and psychological perspectives to obtain an universal inventory of basic colour terms (focal colours) \cite[see e.g.]{berlinkay1969,RegierKayCook2005}. However, naming a colour does not mean perceive a colour, at least during the early stages of childhood {[}VOIR TRAVAUX D'ANNA FRANKLIN{]}, even if it exists works which try to tackle this gap \cite[see \textit{e.g.}][]{parragaakbarinia2016}.
...
the West, whose six from the first rank: white, red, black, green, yellow and blue; and five from the second rank: pink, orange, purple (violet), gray and brown. \cite{berlinkay1969} finds that ... Finally, there are also eleven \textit{hues} in the Munsell colour system, ....

In order to obtain a ground truth, we asked to five human experts (from Swiss, Russia, Iran and Belgium) to look at ... drawings and to select the presence or absence of colours in a list based on the Munsell colour system [REF]... we added colours:...  (it was the second step of the experiment, the first one consisted in writing the colours they saw, see section~\ref{sec:results_diversity} {\color{red}[ADD A LABEL IN 4.2.2 to be more precise]}). However, the agreement between annotators was really low... [RESULTS AND REF] and thus, it was not possible to create a gold standard. This result is not surprising according to the works mentioned above.
...
For instance, when colour is used in image
...
and described above.    \cite{benavente2008}
...
More specifically, the paper of
...
as done by \citet{kimbaelee2007}
...
{\color{red}[Other distances tested, add ref in 4.1 for fig. 3]}
...
{\color{red}[AX: peut-on renommer $f^{g}$ ci-dessus (et ailleurs si necessaire) en $n^{g}$ et l'appeler {\em count}? comme ca on peut reserver la notation $f^{g}$ et le terme {\em frequency} pour les frequences relatives. Dans ma partie ci-dessous je fais d'office ce changement.]}
...
{\color{red}[AX: y a-t-il autre chose a dire a ce stade? il me semble que l'entropie sur les niveaux de gris peut et devrait etre introduite directement dans les resultats, et meme chose pour le fait d'utiliser les quantiles de l'entropie. Si vous etes d'accord, une bonne partie de la fin de la section peut etre supprimee (ou inseree dans la section~\ref{sec:results_diversity} si necessaire)].}
...
{\color{gray}Concerning the first gap, through the \emph{state-of-art} of colour
