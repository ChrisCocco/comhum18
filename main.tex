...
\color{blue}{Confronted to important visual data such as images, researchers in Social sciences and Humanities need computational methods to increase the coverage of their analysis. 
...
The results obtained shown that the method is particularly useful to analysis large image dataset and well responsive to specific questions that belong to our field of study.
}
\end{abstract}
...
{\color{gray} On the top, all colours are identified as expected by the team, which is generally the case. In the middle, again, all colours are identified correctly and as expected, since there is no brown macro-colour, it is divided into orange and red. Also, and it is coherent, blue and yellow together in the centre of the drawing creates green. On the bottom, it is a default of the method for high saturated colours, especially for green.
}
...
For this dataset, the results obtained allows to said that the Achromatic is the most identified followed by the Blue and the Yellow whereas the Pink and the Purple are clearly less present (they represent $71.3\%$ against $0.9\%$). The Orange, Red, green and Cyan seem to be moderately present ($27.8\%$). 
{\color{red} 5) ** P.-Y.B. ** 5)} {\color{gray} Most preferred colours in general are blues and greens such as cyan, as well as red (especially for female and 3- to 4-year old children), while the least preferred colour is yellow (as well as orange and sometimes red [SEULEMENT POUR GRANGER]) \cite[see e.g.][]{Granger1955,Zentner2001,JonauskaiteMohrAntoniettiEtAl2016}. The results obtained show that children certainly do not use their preferred colours to draw god, but specific colours for this task. Assuming that they employ blue to depict the sky and the yellow for the light or the sun, it confirms the idea that children imagine god as something or someone shining in the sky. With this method, it will also be possible to obtain the same type of results for each country and then to compare them or for a an age group, and so on. Thus, it will possible to test various hypothesis of psychology.}
...
{\color{red} 6) ** P.-Y.B. **} {\color{gray} 1) On a le nombre de couleurs, ce dont on avait besoin
2) Ca permet de distinguer les enfants qui gardent le fonds blanc par rapport à ceux qui remplissent la page (donc une composante spatiale)
3) Comprendre le rôle relatif des couleurs dans une composition spatiale
}
