...
\begin{figure}[!h]
	\centering
	\includegraphics[width=0.32\linewidth]{figures/ru08_bo_f_pb_07_05_ali-r.jpg}
	\includegraphics[width=0.62\linewidth]{figures/ru08_bo_f_pb_07_05_ali-rno_filter_mask.pdf}
...
In more details, we observe on Figure \ref{fig:grille} that the entropy values attached to each drawing indicates a degree of completeness: higher is the entropy value, more the colours are covering the page revealing more complex drawings whereas the lower values depicts lower coloured and lower complex drawings - where $H(f) \rightarrow 0$ reveals whiter pages. Also, combined with the variety, we observe that measure allows a gradual distinction between white drawing background with a single object to drawing with more contextualized (mutli-)object(s).
...
\subsubsection{Human Concordance}
